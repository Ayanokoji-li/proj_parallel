...
class CUEFGraph_ForPG : public CUEFGraph<kSkipQuantum, kForwardQuantum>
{
private:
    const double* d_Val;
...
public:
    CUEFGraph_ForPG(const EFGraph_ForPG<kSkipQuantum, kForwardQuantum>& ef_graph, int alloc_mode = 0)
        : CUEFGraph<kSkipQuantum, kForwardQuantum>(ef_graph, alloc_mode), d_Val(ef_graph.GetVal())
    {
    }

    CUEFGraph_ForPG(const EFLayout_ForPG<kSkipQuantum, kForwardQuantum>& ef_layout, int alloc_mode = 0)
        : CUEFGraph<kSkipQuantum, kForwardQuantum>(ef_layout, alloc_mode), d_Val(ef_layout.GetVal())
    {
    }
